...
exports.getVHDInfo =
  ...
  var vhdInfo = {footer : footer, isDiff : diffVhd, fd: fd };
  var ddHeader = null;
  var indices = null;
  ...
  vhdInfo.cleanup = function() {
        if (vhdInfo.fd) {
          fs.closeSync(vhdInfo.fd);
          vhdInfo.fd = null;
        }
      }
  ...
  vhdInfo.getReadStream = function(streamOptions) {
    ...
    function createNextStream(block, startPosWithinBlock) {
      ...
      stream._fileStream.on('end', function onEnd() {
        stream._fileStream.removeAllListeners();
        stream._fileStream = null;
    ...
    stream.pipe = function(dest, options) {
      ...
      dest.emit('pipe', stream);  // Let the request know we are piping to it.
    };
