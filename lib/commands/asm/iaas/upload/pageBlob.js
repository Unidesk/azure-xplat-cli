...
exports.copyBlob = function copyBlob(sourceUri, sourceAccountKey, destinationUri, destinationAccountKey, callback) {
  ...
  blobService.createContainerIfNotExists(container, function(error1, containerCreated, response1) {
    ...
    blobService.startCopyBlob(sourceUri, container, splitDestination.blobName, checkStatus);
...
var signBlobUri = exports.signBlobUri = function signBlobUri(uri, storageAccountKey) {
  ...
  if (storageAccountKey) {
    ...
    var blobService = azureStorage.createBlobService(splitUri.accountName, storageAccountKey);
    ...
    var queryString = blobService.generateSharedAccessSignature(splitUri.container, splitUri.blobName, sharedAccessPolicy);
    uri = uri + '?' + queryString;
  }
...
var uploadPageBlobFromBlobService =
  ...
  var blockSize = Math.min(2 * 1024 * 1024, BlobConstants.DEFAULT_WRITE_BLOCK_SIZE_IN_BYTES);
  ...
  function createDiffVHDPageBlob(blobService, container, blobName, baseUri, threads, vhdInfo, callback) {
    ...
    blobService.startCopyBlob(splitParentUri.container, splitParentUri.blobName, container, blobName, function(error) {
      if (error) {
      ...
      blobInfo.getBlobInfo(blobService, container, blobName, null, false, function(error, result) {
        ...
        function deleteRegion(i) {
          ...
          blobService.clearPageRange(container, blobName, deleteRegions[i].start, deleteRegions[i].end, function(error) {
            if (error) {
    ...
    function next(number) {
      ...
      function uploadAtIndex(index) {
        if (index > maxIndex) { // create a footer
          ...
          var stream = new memoryStream.ReadableStream(vhdInfo.footer.buffer);
          blobService.createPagesFromStream(container, blobName, stream, size - 512, size - 1, toNext);
        } else {
          ...
          blobService.createPagesFromStream(container, blobName,
